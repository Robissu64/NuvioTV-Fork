# Nuvio Center Test V1.2 — ganho ajustável e abertura de vídeos externos

Atualização incremental para a V1.1 que já está funcionando na TV.

## Mudanças

- O toggle fixo foi substituído por **Ganho do canal central**, ajustável de 0 a +6 dB, em passos de 1 dB.
- Migração automática: ao atualizar da V1.1 com o teste ligado, o novo controle começa em **+4 dB**.
- Em 0 dB, a rota experimental AC-3 é desligada; de +1 a +6 dB, ela continua ativa.
- O Nuvio pode ser escolhido em **Abrir com** para vídeos enviados por `content://` ou `file://`, como pelo CX Explorer.
...
- Sem limitador, a central pode ultrapassar o nível digital máximo e distorcer nas etapas seguintes. Não há promessa de ausência de clipping, especialmente acima de +4 dB.
...
7. O nome do workflow e do artefato continua V1, mas o app compilado terá versão 0.9.0-center-v1.2.
...
Mantenha Preferir decodificadores do app, Ganho do canal central em +4 dB, AC-3 capable receiver ligado, downmix desligado e tunneled desligado. Reabra o mesmo filme, selecione a mesma faixa Português AC-3 5.1 e compare o mesmo trecho de ação no mesmo volume. Observe impacto das explosões, separação traseira, distorção e sincronismo.

Para abrir um vídeo pelo CX Explorer: inicie o compartilhamento normalmente, abra o arquivo na TV e selecione **Nuvio Center Test** em **Abrir com**. O player deve abrir diretamente o `content://` recebido.
...
Log esperado: CENTER_TEST_V1_1: AC3/EAC3 decoder DRC disabled; e CENTER_TEST_V1_2: FC +N dB, linear gain, no peak protection.
